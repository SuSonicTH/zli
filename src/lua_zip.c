...
int luaopen_zip(lua_State *L) {
    luaL_newlib(L, lua_ziplib);
    luax_register_mt(L, lua_zip_udatamt);
    return 1;
}
...
int lua_zip_open(lua_State *L) {
    ...
    unz_file_info uzfi;
    char zfname[LUA_ZIP_BUFFERSIZE];
    long fpos = 1;
    int udpos = 0;
    ...
    luax_createudata(L, uzfh, LUA_ZIP_UZ_FILE);
    udpos = lua_gettop(L);
    lua_newtable(L);
    lua_pushstring(L, LUA_ZIP_UZ_FILE);
    ...
    lua_settable(L, -3);
    luax_settable_function_list(L, -3, lua_zip_open_reg);
    ...
    do {
        ...
        // push functions
        luax_settable_function_list(L, -3, lua_zip_open_file_reg);
        lua_settable(L, -5);
        lua_settable(L, -3);
    ...
    lua_zip_filesize_hr(uncompressed, sizehr);
    luax_settable_string(L, -3, "uncompressed_size_hr", sizehr);
    lua_zip_filesize_hr(compressed, sizehr);
    luax_settable_string(L, -3, "compressed_size_hr", sizehr);
    return 1;
}
...
int lua_zip_lines_mt(lua_State *L) {
    ...
    lua_gettable(L, -2);
    return lua_zip_lines(L);
}

int lua_zip_lines(lua_State *L) {
    unzFile uzfh;
    const char *fname;
    luax_getarg_gcudata(L, 1, LUA_ZIP_UZ_FILE, unzFile, uzfh, "Zip object expected");
    ...
    lua_pushcfunction(L, lua_zip_file_uzread);
    lua_newtable(L);
    lua_pushstring(L, LUA_ZIP_UZ_FILE);
    lua_pushstring(L, LUA_ZIP_UZ_FILE);
    lua_gettable(L, 1);
...
int lua_zip_open_uzfile(lua_State *L) {
    ...
    lua_gettable(L, 1);
    lua_settable(L, -3);
    luax_settable_cfunction(L, -3, "read", lua_zip_file_uzread);
    luax_settable_cfunction(L, -3, "close", lua_zip_file_uzclose_current);
    luax_settable_cfunction(L, -3, "eof", lua_zip_file_uzeof);
    luax_settable_cfunction(L, -3, "tell", lua_zip_file_uztell);
    return 1;
}
...
int lua_zip_file_uzread(lua_State *L) {
    ...
    for (n = 2; n <= maxarg; n++) {
        ...
        // TODO: lua_zip_file_uzread: Implement buffered file reading and missing read modes
        switch (readmode[1]) {
            case 'n':
                break;
            case 'a':
                luaL_buffinit(L, &lb);
                while ((cr = unzReadCurrentFile(uzfh, &rb, LUA_ZIP_BUFFERSIZE)) > 0) {
            ...
            case 'c':
                luaL_buffinit(L, &lb);
                while ((cr = unzReadCurrentFile(uzfh, &rb, chartoread > LUA_ZIP_BUFFERSIZE ? LUA_ZIP_BUFFERSIZE : chartoread)) > 0) {
                    chartoread -= cr;
                    luaL_addlstring(&lb, rb, cr);
                }
                luaL_pushresult(&lb);
...
int lua_zip_file_uzclose(lua_State *L) {
    ...
    unzCloseCurrentFile(uzfh);
    unzClose(uzfh);
    luax_set_gcudata(L, 1, LUA_ZIP_UZ_FILE, NULL);
    return 0;
}
...
int lua_zip_extract_file(lua_State *L) {
    ...
    while ((cr = unzReadCurrentFile(uzfh, &buff, LUA_ZIP_BUFFERSIZE)) > 0) {
    ...
    do {
        ...
        if (zfname[strlen(zfname) - 1] == '/') {
            if (dest) {
                ...
                lua_pop(L, 1);
                if (!lua_zip_createtree(path)) {
                    lua_pushnil(L);
                    lua_pushfstring(L, "Could not create output path:'%s'", path);
                    return 2;
                }
            } else {
                if (!lua_zip_createtree(zfname)) {
                    lua_pushnil(L);
...
int lua_zip_createtree(const char *path) {
    ...
    if (path[strlen(path) - 1] == '/') {
        ...
        *cpos = 0;
        return lua_zip_createtree(npath);
...
int lua_zip_uzfile_gc(lua_State *L) {
    unzFile uzfh;
    luaL_checkudata(L, 1, LUA_ZIP_UZ_FILE);
...
int lua_zip_create(lua_State *L) {
    ...
    lua_newtable(L);
    lua_pushstring(L, LUA_ZIP_ZIP_FILE);
    luax_createudata(L, zfh, LUA_ZIP_ZIP_FILE);
    lua_settable(L, -3);
    luax_settable_cfunction(L, -3, "close", lua_zip_zipfile_close);
    luax_settable_cfunction(L, -3, "addfile", lua_zip_addfile);
    luax_settable_cfunction(L, -3, "open", lua_zip_open_zipfile);
    return 1;
}

int lua_zip_zipfile_gc(lua_State *L) {
    zipFile zfh;
    luaL_checkudata(L, 1, LUA_ZIP_ZIP_FILE);
...
int lua_zip_zipfile_close(lua_State *L) {
    zipFile zfh;
    const char *comment;
    luax_getarg_gcudata(L, 1, LUA_ZIP_ZIP_FILE, zipFile, zfh, "Zip object expected");
    comment = lua_tostring(L, 2);
    zipCloseFileInZip(zfh);
    zipClose(zfh, comment);
    luax_set_gcudata(L, 1, LUA_ZIP_ZIP_FILE, NULL);
    return 0;
}

int lua_zip_zip_fill_filedate(lua_State *L, zip_fileinfo *zfi, const char *filename, int n) {
    char *stime;
    const char *serror;
...
int lua_zip_addfile(lua_State *L) {
    ...
    const char *outfname;
    const char *comment;
    char buffer[LUA_ZIP_BUFFERSIZE];
    FILE *fh;
    size_t cr;
    int compression;
    ...
    if (!lua_zip_zip_fill_filedate(L, &zfi, infname, 4))
        return 2;
    ...
    while ((cr = fread(buffer, 1, LUA_ZIP_BUFFERSIZE, fh)) > 0) {
...
int lua_zip_open_zipfile(lua_State *L) {
    ...
    luaL_checkstring(L, 2);
    fname = lua_tostring(L, 2);
    if (!lua_zip_zip_fill_filedate(L, &zfi, NULL, 3))
        return 2;
    ...
    lua_gettable(L, 1);
    lua_settable(L, -3);
    luax_settable_cfunction(L, -3, "write", lua_zip_file_zipwrite);
    luax_settable_cfunction(L, -3, "close", lua_zip_file_zipclose);
